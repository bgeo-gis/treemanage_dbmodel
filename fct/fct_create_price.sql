...
--check if the new camapaign inserted by the user already exists and if the capaign from which the data should be copied exists
	select exists (select distinct campaign_id from cat_price where campaign_id=new_campaign_aux) into exist_new;
	select exists (select distinct campaign_id from cat_price where campaign_id=old_campaign_aux)into exist_old;
...
--	If there is no existing campaign, create new rows for combination of work-size-new campaign_id with all prices=0,00
 		INSERT INTO cat_price(work_id,size_id, campaign_id,price) SELECT DISTINCT cat_work.id, cat_size.id,new_campaign_aux,0 FROM cat_work, cat_size order by 1,2;
...
--if the new and old campaign exists: Remove all the existing new campaign_id data and copy the existing ones of the old campaign_id 		
 		...
 		INSERT INTO cat_price(work_id,size_id, campaign_id, price) SELECT DISTINCT work_id, size_id, new_campaign_aux, price FROM cat_price
